...
(function () {
  ...
  const TOPBAR_SUPABASE_URL = 'https://wonypilgbyuglfhbredl.supabase.co';
  const TOPBAR_SUPABASE_KEY = 'sb_publishable_dILS-NOpuY0XHDxsw4eV-w_Vz4_ycXp';
...
/* === Global mobile lockdown ===
   1) Hide the right-side scrollbar on phones.
   2) Stop iOS auto-text-size-adjust.
   3) touch-action: pan-y prevents pinch-zoom while still allowing vertical scroll.
   4) overscroll-behavior on every known modal class stops scroll chaining —
...
`;
  ...
  async function pushWaterMergedToSupabase(localWater) {
    // Only do this when NOT on the health page — health page has its own sync.
